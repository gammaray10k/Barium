    ...
    ui(new Ui::ProcessModeUI)
{
    ...
    connect(ui->twoPassEncode, &QCheckBox::clicked,
            this, &ProcessModeUI::two_pass_encode);*/
    //--------------------------------------------------------------------------
    ...
    ui->twoPassEncode->setToolTip(tr("Two-pass encoding"));
...
void ProcessModeUI::two_pass_encode()
{
    if(ui->twoPassEncode->isChecked())
    {
        ui->extractAudioRadio->setCheckable(false);
        ui->extractAudioRadio->setToolTip(tr("Cannot two-pass encode audio"));
        ui->ffProcWindow->setText(ff_output+"No two-pass implementation yet!");
        ui->statusbar->showMessage(tr("No two-pass implementation yet!"));
    }
    else
    {
        ui->extractAudioRadio->setCheckable(true);
        ui->extractAudioRadio->setFocus();
        ui->extractAudioRadio->setToolTip(tr("Extract audio from video to "
                                             "save"));
    }
}*/
